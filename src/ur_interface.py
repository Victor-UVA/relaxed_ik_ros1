...
class end_effector():
    def __init__(self):
        self.angular_pose_pub = rospy.Publisher("ur/ur_arm_scaled_pos_joint_traj_controller/command", JointTrajectory, queue_size=10)
        self.joint_states_sub = rospy.Subscriber("joint_states",JointState,self.js_cb)
        self.rleaxed_ik_joint_angles = rospy.Subscriber("relaxed_ik/joint_angle_solutions",JointAngles, self.rik_ja_cb) 
    ...
    def send_transforms(self):
        ...
        ee_link = self.transform(self.pose_lookup("ur_arm_wrist_2_link","ur_arm_flange"),"ur_arm_wrist_2_link","ur_arm_ee_link",0,-1.5707,1.5707)
        starting_pose = self.transform(self.pose_lookup("ur_arm_base_link","ur_arm_flange"),"ur_arm_base_link","ur_arm_starting_pose",0,-1.5707,3.14159)
    ...
    def transform(self,pose,parent,child,x,y,z):
        if pose is not None:
            ...
            static_transformStamped.transform.translation.x = float(pose.transform.translation.x)
            static_transformStamped.transform.translation.y = float(pose.transform.translation.y)
            static_transformStamped.transform.translation.z = float(pose.transform.translation.z)
...
if __name__ == '__main__':
    ...
    rospy.set_param('ur/ur_arm_scaled_vel_joint_traj_controller/constraints/goal_time',10)
    # rate1 = rospy.Rate(1)
