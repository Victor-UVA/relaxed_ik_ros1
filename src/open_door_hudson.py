...
def calculate_collision_angle(door_width, distance_from_door): #returns degrees
    sigma_r = 0.03  # m
    sigma_d = 0.05  # m
        ...
        if arm.state == States.PLANNING:
            ...
            for i in range(len(angles)):
                if angles[i] >= collision_angle:
                    ...
                    break
            rospy.loginfo("Door will collide at " +str(collision_angle)+" / at index "+str(collision_index))
            ...
            arm.send_goal(handle_arc[collision_index-1][0], handle_arc[collision_index-1]
                          [1], handle_arc[collision_index-1][2]+0.1, y_q=0)
            rospy.sleep(2)
            arm.send_goal(0, 0, 0)
            rospy.sleep(10)
